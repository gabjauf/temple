...
const settings = (options.values && fsSync.existsSync(options.values) ? JSON.parse(fsSync.readFileSync(options.values).toString()) : JSON.parse(options.value)) || (await inquirer.prompt(config.prompt));
