...
import { evalInContextIsolated } from './evalInContext-Isolated';

describe('evalInContextIsolated', () => {
  test('should perform string concatenation', async function () {
    const code = '"app" + "le"';
    const evaluated = await evalInContextIsolated(code);
  ...
  test('should be able to use async function', async function () {
    const code = `
      Promise.resolve('Hey');
    `;
    expect(await evalInContextIsolated(code)).toBe('Hey');
  });
});
